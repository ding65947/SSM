 ...
 * 2、通过控制器方法的形参获取请求参数
 * 只需要在控制器方法的形参位置，设置一个和请求参数名字一致的形参即可
 * 3、@RequestParam:将请求参数和控制器方法的形参绑定
 * @RequestParam注解的三个属性：value,required,defaultValue
 * value:设置和形参绑定的请求参数的名字
 * required：设置是否必须传输value所对应的请求参数，
 * 默认值为true，表示value所对应的请求参数必须传输，否则页面报错400
 * 若设置为false,则表示value对应的请求参数不是必要传输，若未传输，则形参值为null
 * defaultValue：当没有传输Value所对应的请求参数时，为形参设置的默认值
...
public class TestParamController {
    ...
    public String getParam(@RequestParam(value = "userName",
            required = true,defaultValue = "hello") String username,
                           String password,
